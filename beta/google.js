...
function doPost(e) {
    ...
    try {
        ...
        if (!spreadsheetId) {
            throw new Error("Spreadsheet ID not found. Ensure `intialSetup` has been run.");
        }
        ...
        let rowIndex = data.findIndex((row, index) => index > 0 && row[agentIDIndex] == agentID);
        ...
        if (rowIndex === -1) {
            ...
            rowIndex += 1;
            const existingRowData = sheet.getRange(rowIndex, 1, 1, newRow.length).getValues()[0];
            const updatedRow = existingRowData.map((value, idx) => newRow[idx] || value);
            sheet.getRange(rowIndex, 1, 1, updatedRow.length).setValues([updatedRow]);
            SpreadsheetApp.flush();
            Logger.log("Updated existing row for agentID: " + agentID);
        }
        return ContentService.createTextOutput(JSON.stringify({ result: "success", row: rowIndex === -1 ? "New Row" : rowIndex })).setMimeType(ContentService.MimeType.JSON);
    } catch (error) {
