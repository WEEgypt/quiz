...
function signin() {
    ...
    store = document.getElementById("store").options[document.getElementById("store").selectedIndex].text;
    area = document.getElementById("store").value;
    if (agentName == "admin") {
        agentName = document.getElementById("agent").value + "_" + Date.now() + "_" + Math.floor(Math.random() * 1000);
        store = "-";
        area = "-";
        ...
        welcomeAgent();
    } else if (agentName !== "admin" && agentName !== "" && agentName.length >= "8" && store !== "Store Name") {
        login.style.display = "none";
        welcome.style.display = "block";
        footer.style.display = "block";
        welcomeAgent();
...
function welcomeAgent() {
    ...
    document.getElementById("welcomeMsg").innerHTML = "Welcome " + firstName + ", <br /><br /> This Quiz consists of 10 Questions, with 10 Minutes time frame, only One attempt available.";
}
function signout() {
    location.reload();
}
